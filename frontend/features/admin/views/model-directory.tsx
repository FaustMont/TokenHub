          ...
          {models.map((model) => {
            ...
            return (
              <tr key={model.name}>
                ...
                <td>{model.modality === "rerank" && model.metadata?.search_unit_price_usd?.trim() ? <><strong>${formatMoney(Number(model.metadata.search_unit_price_usd))}</strong><span>{tx("搜索单元价格 USD/次")}</span>{model.input_price_usd_per_1m != null ? <span>{tx("输入")} · {priceMetric(model.input_price_usd_per_1m)}</span> : null}</> : <><strong>{priceMetric(model.modality === "embedding" ? model.embedding_price_usd_per_1m : model.input_price_usd_per_1m)}</strong><span>{model.modality === "embedding" ? "Embedding" : tx("输入")}{model.modality !== "embedding" && model.modality !== "rerank" ? <> · {priceMetric(model.output_price_usd_per_1m)} {tx("输出")}</> : null}</span></>}</td>
                {!readOnly ? (
