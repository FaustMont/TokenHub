...
export function permissionDeniedMessage(target: string) {
  ...
  if (activeLanguage === "ru") {
    return `У этого аккаунта нет прав доступа к ${label}. Данные вне вашей области доступа скрыты; при необходимости обратитесь к администратору для настройки роли или участия в проекте.`;
  }
...
export function permissionPartialLoadMessage(labels: string[]) {
  ...
  const separator = activeLanguage === "en" || activeLanguage === "ru" ? ", " : "、";
  ...
  if (activeLanguage === "ru") return `Скрыто из-за недостатка прав: ${summary}. На этой странице отображаются только доступные вам данные.`;
