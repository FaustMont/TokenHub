...
パッケージは `plugin.yaml` で manifest schema `2` と Plugin API `v2` を宣言します。schema `1` と API `v1` は互換アダプターを通じて引き続きサポートします。権限は呼び出しに投影できる Core データと、Core が受け入れる構造化変更を制限します。これは OS サンドボックスではありません。コマンドポリシーではネットワークとリソースの強制隔離は現在 `unsupported` です。そのため、ランタイムから読み込まれた外部ゲートウェイコマンドの実行は起動前に拒否されます。リクエストチェーンのプラグインにはプロセス、ネットワーク、リソースの強制隔離が必要です。この拒否がリクエストに与える影響は Hook の失敗ポリシーで決まり、プロセス内の組み込みプラグインには影響しません。その他の外部コマンドには制限された環境変数、パッケージ相対の実行パス、入出力上限、タイムアウトを適用しますが、ホストレベルのネットワーク／リソース隔離はありません。汎用コマンドの既定値は 30 秒、入力 4 MiB、出力 1 MiB で、各能力面には独自のタイムアウトがあります。例えばゲートウェイフックの既定値は 5 秒です。ストリーミング対応はアダプターごとに確認が必要です。現在の外部 Provider ブリッジはコマンド結果のイベント配列を解析し、子プロセスの出力をリアルタイムに中継するものではありません。
