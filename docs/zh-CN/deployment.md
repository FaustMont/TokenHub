...
`data/model-catalog.yaml` 提供跟踪目录的参考元数据，它不是路由准入清单，也不会发布模型。`data/provider-catalog.json` 提供 Provider 模板，以及在 Provider 配置中可选择的上游模型。引入选中项只会创建持久化的 Provider 模型库存；对外模型及其统一对客价格需要在模型目录中单独创建，再到路由策略映射到已引入的 Provider 模型。`GET /v1/models` 只返回启用且至少存在一条启用路由的对外模型；配置 API Key 模型白名单时还会进一步过滤。`TOKENHUB_PLUGIN_DIR` 指向后端启动时扫描的持久化插件包目录。Docker Compose 部署中该目录由 `tokenhub-plugins` volume 承载，所以插件市场安装结果和插件启停状态会在镜像升级后保留。 容器入口脚本会校验 `TOKENHUB_PLUGIN_DIR` 为非根目录的绝对路径，并在降低 root 权限前创建目录、将其所有者设为运行时的 `node` 用户，首次使用新卷时也会执行。`TOKENHUB_PLUGIN_MARKETPLACE_URL` 可以指向一个 HTTPS JSON 索引，用于在管理后台浏览并安装插件包。如需为启动加载和刷新回退使用自定义 Provider 目录，将 `TOKENHUB_PROVIDER_CATALOG_FILE` 指向具有相同 `providers` 结构的本地 JSON 文件。
