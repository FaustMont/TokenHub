...
TokenHub 会将通过验证的插件包解压到 `TOKENHUB_PLUGIN_DIR`，并在安装、更新、启用、禁用或回滚后重新加载插件运行时。这些操作通常不需要重启 TokenHub 服务。生命周期事实相互独立：已安装、已启用、已配置、使用中和需要重启不能再由一个聚合状态推断。只有期望状态与实际运行状态确实不一致时才显示 `restart_required`；服务成功启动并加载期望插件状态后会清除该标记。之后应验证：
